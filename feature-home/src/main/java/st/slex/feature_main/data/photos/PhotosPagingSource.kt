...
) : PagingSource<Int, st.slex.core_network.model.ui.image.ImageModel>() {

    override fun getRefreshKey(state: PagingState<Int, st.slex.core_network.model.ui.image.ImageModel>): Int? {
        val anchorPosition = state.anchorPosition ?: return null
        val anchorPage = state.closestPageToPosition(anchorPosition) ?: return null
        return anchorPage.prevKey?.plus(1) ?: anchorPage.nextKey?.minus(1)
    }

    override suspend fun load(params: LoadParams<Int>): LoadResult<Int, st.slex.core_network.model.ui.image.ImageModel> {
